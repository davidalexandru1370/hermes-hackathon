...
export default function SideBarMenu() {
  ...
  const getAllDataFromEmployes = async () => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const isFound = rows.some((element) => {
        const name: string = element.documents![0].id.split("/").slice(-1)[0];
        console.log("HERE", name, doc.id);
        if (name[0] == doc.id) return true;
        return false;
      });
      if (isFound) storingArray.push({ data: doc.data(), id: doc.id });
    });
  ...
  const [employesInNeedOfNewDocuments, setEmployeesInNeedOfNewDocuments] =
    useState([]);
  console.log("RED EMPLOYES", employesInNeedOfNewDocuments);

  console.log(numberOfExpiredDocuments);
  console.log("emploes side bar menu", employes);
      ...
      >
        ...
        >
          ...
          >
            ...
            <Tab
              icon={
                <PersonIcon sx={{ paddingRight: "8px" }} fontSize="small" />
              }
              iconPosition="start"
              label="Employees"
              {...a11yProps(1)}
        ...
        <Box>
          ...
          >
            ...
            <Typography pl="24px" mt="24px">
              You have{" "}
              <span
                style={{
                  color: `${numberOfExpiredDocuments > 0 ? "red" : "inherit"}`,
                }}
              >
                {numberOfExpiredDocuments}
              </span>{" "}
              employees in need of medical checkups
            </Typography>
            <Typography
              color="red"
              pl="4px"
          ...
          <TabPanel value={value} index={0}>
            <EmployesList
              numberOfExpiredDocuments={numberOfExpiredDocuments}
              setNumberOfExpiredDocuments={setNumberOfExpiredDocuments}
              employesInNeedOfNewDocuments={employesInNeedOfNewDocuments}
              setEmployeesInNeedOfNewDocuments={
                setEmployeesInNeedOfNewDocuments
              }
              getAllDataFromEmployes={getAllDataFromEmployes}
              employes={employes}
              rows={rows}
              setRows={setRows}
            />
          ...
          <TabPanel value={value} index={1}>
            ...
            >
              {employes.map((employee: any) => (
                <>
                  <Employee
                    employesInNeedOfNewDocuments={employesInNeedOfNewDocuments}
                    avatar={avatars[Number(employee.data.employeId) - 1]}
                    employee={employee}
                  ></Employee>
          ...
          <TabPanel value={value} index={2}>
            ...
            >
              {employesInNeedOfNewDocuments.map((employee: any) => (
                <>
                  <EmployeeRed
                    employee={employee}
                    avatar={avatars[Number(employee.employeId) - 1]}
                  ></EmployeeRed>
                  {/* <h2>{employee.data.employeId}</h2>
          <h2>{employee.data.employeName}</h2>
          <h2>{employee.data.employeDepartment}</h2>
